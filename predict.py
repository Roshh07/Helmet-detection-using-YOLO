...
def predictor_two_wheeler(image):
    ...
    img_copy=img.copy()
    result1=model1.predict(img,show_boxes=True,iou=0.7)
    ...
    if cls_m2.count(1)==0:
        ...
        for cls_,b_box in zip(cls_m2,bounding_boxes):
            ...
            if cls_ == 1:
                ...
                dh,dw,_=image.shape
                lx,ly,rx,ry=b_box_cord(b_box,dh,dw)
                p=cv2.rectangle(image,(lx,ly),(rx,ry),(255,0,0),3)
                k=k+1
...
def select_vehicle():
    ...
    global dict
    return count,dict
bole=predictor_two_wheeler(r"Untitled.jpg")
im=PIL.Image.open(r"untitled.jpg")
